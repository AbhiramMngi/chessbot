...
class MCTS:
    ...
    def __init__(self, predict: Callable, root: Node = None, color: int = 1, mode: str = "train"):
        ...
        self.board = chess.Board(self.root.state)
        self.mode = mode
        self.move_stack = []
    ...
    def select_leaf(self, root: Node) -> Node:
        ...
        best_child = self.root.children[dirichlet.argmax()]
    ...
    def expand_node(self, leaf: Node):
        ...
        for i, move in enumerate(leaf.legal_moves):
            ...
            if max_node is None or max_node.position_score() < new_node.position_score() * dirichlet[i]:
                max_node = new_node
                max_move = move
            self.board.pop()
